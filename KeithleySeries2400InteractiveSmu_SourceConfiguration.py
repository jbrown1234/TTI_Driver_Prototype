...
class SourceConfiguration:
    ...
    def update_comms(self):
        """
        This function is used to ensure lower level consumer classes tied\
        to the driver are updated to promote instrument communications.
        """
    ...
    class ConfigList:
        ...
        def delete(self, list_name, index=None):
            ...
            """
            if index is None:
                self._mycomms.write(f"smu.source.configlist.delete(\
                    \"{list_name}\"))")
            else:
                self._mycomms.write(f"smu.source.configlist.delete(\
                    \"{list_name}\", {index}))")
        ...
        def query(self, list_name, index, field_separator=None):
            """
            This function returns a list of TSP commands and parameter\
            settings that are stored in the specified configuration index.

            :param list_name: A string that represents the name of a source\
                configuration list
            ...
            if field_separator is None:
                self._mycomms.query(f"print(smu.source.configlist.query(\
                    \"{list_name}\", {index}))")
            else:
                self._mycomms.query(f"print(smu.source.configlist.query(\
                    \"{list_name}\", {index}, {field_separator}))")

        def recall(self, list_name, index=None, source_list_name=None, 
                   source_index=None):
            """
            This function recalls a configuration index in a source\
            ...
            if source_index is None:
                if index is None:
                    self._mycomms.write(f"smu.source.configlist.recall(\
                        \"{list_name}\")")
                else:
                    self._mycomms.write(f"smu.source.configlist.recall(\
                        \"{list_name}\", {index})")
                ...
                if source_index is None:
                    self._mycomms.write(
                        f"smu.source.configlist.recall(\"{list_name}\",\
        ...
        def size(self, list_name):
            ...
            of a source configuration list.

            :param list_name:
            :return:
            """
            self._mycomms.write(f"index_count = smu.source.configlist.zier(\
                \"{list_name}\")")
            return int(self._mycomms.query("print(index_count)"))

        def store(self, list_name, index=None):
            """
            This function stores the active source settings into the named\
                configuration list.
            ...
            """
            if index is None:
                self._mycomms.write(f"smu.source.configlist.store(\
                    \"{list_name}\")")
            else:
                self._mycomms.write(f"smu.source.configlist.store(\
                    \"{list_name}\", {index})")
        ...
        def storefunc(self, list_name, function, index=None):
            """
            This function allows you to store the settings for a source\
                function into a source configuration \
            ...
            if index is None:
                self._mycomms.write(f"smu.source.configlist.storefunc(\
    ...
    def func(self):
        """
        This attribute contains the source function, which can be voltage\
        or current.
        """
    ...
    def level(self):
        """
        This attribute immediately selects a fixed amplitude for the\
    ...
    class ILimit:
        ...
        def level(self):
            """
            This attribute selects the source limit for current\
            measurements.
            """
        ...
        def tripped(self):
            """
            This attribute indicates if the source exceeded the limits that\
            were set for the selected measurements.
            """
    ...
    def offmode(self):
        """
        This attribute defines the state of the source when the output is\
        turned off.

            NORMAL = 2-wire sense, voltage source with 0 V level, current\
                limit level set to 10% full scale of present range
            HIGHZ = 2-wire sense, output relay opens
            ZERO = 2-wire sense, voltage source with 0 V level, if current\
                measure set then current limit level is not
                changed otherwise the measurement is set to current and\
                limit level is set to 10% full scale of present range
            GUARD = 2-wire sense, current source selected and set to 0 A\
                level, voltage limit level set to 10% full scale of present\
                range

        :return: Either OFFMODE_NORMAL, OFFMODE_HIGHZ, OFFMODE_ZERO, or\
            OFFMODE_GUARD
        """
        self._mycomms.write("source_off_mode = smu.source.offmode")
        source_off_mode = self._mycomms.query("print(\
            source_off_mode)").rstrip()
    ...
    def offmode(self, source_off_mode):
        ...
        turned off.
            ...
            HIGHZ = 2-wire sense, output relay opens
            ZERO = 2-wire sense, voltage source with 0 V level, if current\
                measure set then current limit level is not changed otherwise\
                the measurement is set to current and limit level is set to\
                10% full scale of present range
            GUARD = 2-wire sense, current source selected and set to 0 A\
                level, voltage limit level set to 10% full scale of present\
                range

        :param source_off_mode: Either OFFMODE_NORMAL, OFFMODE_HIGHZ,\
            OFFMODE_ZERO, or OFFMODE_GUARD
        :return:
        """
    ...
    def readback(self):
        """
        This attribute determines if the instrument records the measured\
        source value or the configured source value when making a measurement.
    ...
    def readback(self, state):
        """
        This attribute determines if the instrument records the measured\
        source value or the configured source value when making a\
        measurement.
    ...
    class VLimit:
        ...
        def level(self):
            """
            This attribute selects the source limit for voltage\
            measurements.
            """
